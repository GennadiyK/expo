...
/**
 * Main entry point to expo-updates in development builds with expo-dev-client. Similar to UpdatesController
 ...
 * class differs enough that it is implemented independently from UpdatesController.
 *
...
class UpdatesDevLauncherController(
  context: Context,
  initialUpdatesConfiguration: UpdatesConfiguration?, // TODO(wschurman): remove
  ...
  private var defaultSelectionPolicy: SelectionPolicy = SelectionPolicyFactory.createFilterAwarePolicy(
    initialUpdatesConfiguration?.let { UpdatesUtils.getRuntimeVersion(it) } ?: "1" // matches the default in UpdatesConfiguration
  )
  ...
  override fun getConstantsForModule(context: Context): UpdatesModuleConstants {
    return UpdatesModuleConstants(
      ...
      runtimeVersion = updatesConfiguration?.runtimeVersion ?: "1",
